...
class metodos:
    def whois():
        domain=input("%s%s--------------------------->dominio:%s "%(start,B,r));cls()
        ...
        match rsp.status_code:
            case 200:
                metodos.ndict(rspj)
                print("%s%sPeço desculpas por não traduzir as respostas, veja porque não traduzir em: https://fln.app/flood_ajuda%s"%(start,Y,r))
    ...
    def cep():
        _cep=input("%s%s------------------------------------> cep :%s "%(start,B,r));cls()
        cep=[]
        for remove in _cep:
    ...
    def vt():
        _cll=input("%s%s-------------------------->tel:%s "%(start,B,r))
        cls()
        cll=[]
        for remove in _cll:
    ...
    def cnpj():
        _cnpj=input("%s%s------------------------------>cnpj:%s: "%(start,B,r));cls()
        cnpj=[]
        for remove in _cnpj:
        ...
        if cnpj=="":print();input("\n%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
            ...
            input("\n%s%sAperte a tecla %s%sEnter%s%s para voltar.%s"%(start,B,r,G,r,B,r))
    def ddd():
        ddd=input("%s%s------------------------->ddd%s: "%(start,B,r));cls()
    ...
    def ip():
        ip=input("%s%s-------------------------->ip%s: "%(start,B,r));cls()
...
def menu():
    ...
    while True:
        ...
        cls()
        print(" ╭"+"─"*(size)+"╮")
        print(" │"+str("S.O.D_CONSULTA").center(size)+"│")
        print(" ╞"+"═"*(size)+"╡")
        _menu()
        print(" │","│".rjust(size))
        print(" │","[99] FREE","│".rjust(size-11))
        print(" │","[0] RESERT","│".rjust(size-9))
        print(" ├"+"─"*(size)+"╯")
        ipt = input(f" ├{start}--------------------->opção{r} ")
        cls()
        try:
            ipt = int(ipt)
            if ipt not in _options:
                cls()
                print(f"{start}--------------------->opção{r}")
                sleep(2)
                cls()
        except:
            cls()
        ...
        match ipt:
            ...
            case 6:
                metodos.vt()
            case 0:
                exit("%sCRIADO POR @LRRN_021/FLOOD_OFC!%s"%(start,r))
            case 99:
                apisusadas={
                    "IP": "ip-api.com",
